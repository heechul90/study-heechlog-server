package com.woorinpang.settlementservice.domain.store.common.domain;
...
public class StorePaymentAmount {
    ...
    @AttributeOverrides(@AttributeOverride(name = "value",column = @Column(
            name = "userPayAmount",
            columnDefinition = "bigint default 0 comment '사용자 결제금액'")))
    private Amount userPayAmount;*/

    /*@Embedded
    @AttributeOverrides(@AttributeOverride(name = "value",column = @Column(
            name = "mypointPayAmount",
            columnDefinition = "bigint default 0 comment '마이포인트 결제금액'")))
    private Amount mypointPayAmount;*/

    /*@Embedded
    @AttributeOverrides(@AttributeOverride(name = "value",column = @Column(
            name = "instantPayAmount",
            columnDefinition = "bigint default 0 comment '즉시 결제금액'")))
    ...
    /*@Embedded
    @AttributeOverrides(@AttributeOverride(name = "value",column = @Column(
            name = "couponPayAmount",
            columnDefinition = "bigint default 0 comment '쿠폰 결재금액'")))
